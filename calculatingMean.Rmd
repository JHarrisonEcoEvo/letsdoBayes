...
Cool, next we have our priors, which are statements regarding how probable we think the model is. Our model is a normal, with a mean mu and some variance, so our priors are two probability distribution that tells us how probable different values for mu and sigma are. We see that, in the above example, we have a prior for mu that is a normal distribution centered at 1.7 with 0.3 as the standard deviation and sigma has a prior of a normal distribution centered at zero with a standard deviation of one. 
...
fitstan_HMC <- sampling(model,
                        data = list(n = length(y),
                                ...
                                verbose = T)
```
For some reason, on my desktop only one chain runs without errors and only if I ask for more than one core.  I do not have time to debug this, and for our purposes we can move forward. This is real life. It is almost guaranteed that when you start installing and using Stan you will hit some errors (unless maybe if you have a brand new computer and use the latest versions of everything).
...
The point of having "warmup" (sometimes called "burn-in") samples is that we have to give time for the sampler to converge. Say our first couple samples were terrible and made the model really improbable (like a mean of a million), it won't take too long for the sampler to propose a value that makes the model more probable. We keep doing this until the model converges on good values for the parameters and then starts to sample around those values, thus doing a good job characterizing the posterior (see figures below). This is also why we typically want multiple chains of samples, each of which started with different numbers. That way each chain converges and explores the posterior indepdently. We can put the chains together at the end if we want.

Lets look at our chains for our parameters mu and sigma
```{r}
params <- extract(fitstan_HMC) #get our samples
...
The plots on the left are called trace plots and histograms of the chains are on the right. We see that the trace plots vary around some mean and don't seem to be trending any direction. This means we have converged on a parameter estimate that makes the model probable. 

We typically want to doublecheck via some simple statistics that convergence has happended. Shinystan has some great tools for this. Sinystan is an R package, see here: http://mc-stan.org/shinystan/index.html
...
Note, that because of the bug on my system, I am not sure if rhat was calculated properly. Doesn't matter for now though. 
