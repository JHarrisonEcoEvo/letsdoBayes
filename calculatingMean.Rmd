...
author: Joshua Harrison, Andrew Seifert, with input from Eryn McFarlane and Christopher Weiss-Lehman
---
...
Note, that if using PDFs is an unfamiliar concept, then it would be worth googling that a bit and getting familiar with it. I just scanned this resource and it may be helpful (though a vast many others exist too): https://www.econometrics-with-r.org/2-1-random-variables-and-probability-distributions.html
...
We have our model specified, we will loop through our data, calculate the likelihood of each datum and multiply those together to get the full likelihood of our data given the model, then multiply all that by our prior probabilities for the model (look at Bayes theorem again, we are doing that). 
...
Stan files need to end in a blank line! 

As per usual, be wary of formatting (have semicolons to end lines, etc.). Find examples online and crib their syntax. Stan gives pretty good error messages most of the time, but various compiler errors can be cryptic. For a brand new model, be prepared to debug a bit.
...
Because shinystan is a shiny app it doesn't work to put it in this markdown, but one would call it like this: shinystan(yourModelObject) from within R (after installing the shinystan package and loading it). We can also get a lot of info by printing the model object to the screen, like this: 

```{r}
fitstan_HMC
```

This helpful summary tells us the name of the model, info on chains, including warm up and thinning (how many samples are removed, so that samples are not too autocorrelated, see the Stan help). 
